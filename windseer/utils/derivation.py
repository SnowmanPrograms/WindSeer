...
def derive(input_tensor, deriv_axis, ds=1, terrain=None, no_nans=True):
    '''
    Computing the derivative of a scalar field in the specified axis direction.

    Parameters
    ----------
    input_tensor : torch.Tensor
        Input tensor [samples, Z, Y, X]
    deriv_axis : int
        Axis along which to compute the derivative
    ds : float, default: 1.0
        Grid size of deriv_axis
    terrain : torch.Tensor or None
        Terrain tensor with the same shape as the input_tensor.
        If not none used to ignore the derivation next to the terrain cells.
    no_nans : bool
        If True all nans in the output due to the terrain mask will be removed

    Returns
    -------
    deriv_tensor : torch.Tensor
        Derived field with respect to the deriv_axis for each sample
    '''
...
def gradient(input_tensor, grid_size, terrain=None):
    ...
    ----------
    input_tensor : torch.Tensor
        Input tensor [samples, input_field(u_in, v_in, w_in), Z, Y, X]
    grid_size : list of float
        Grid sizes in X, Y and Z
    terrain : torch.Tensor
        5D tensor containing terrain channel

    Returns
    -------
    gradient_tensor : torch.Tensor
        5D tensor [samples, gradient_components(9 in total), Z, Y, X]
    '''
...
def divergence(input_tensor, grid_size, terrain=None):
    ...
    Returns
    -------
    divergence_tensor : torch.Tensor
        4D tensor of divergence of input field [samples, Z, Y, X]
    '''
