...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        'Plot the wind from a log file and corresponding surrounding COSMO estimates'
        )
    parser.add_argument(
        'yaml_file',
    ...
    parser.add_argument(
        '-s', '--save-figs', action='store_true', help='Save output figures'
        )
    parser.add_argument(
        '-r', '--resolution', type=int, default=64, help='Extracted block resolution'
        )
    parser.add_argument(
        '-p', '--prediction', action='store_true', help='Plot network prediction'
        )
    parser.add_argument(
        '--polar', action='store_true', help='Plot polar coordinates (instead of N,E)'
        )
    args = parser.parse_args()

    cosmo_args = utils.COSMOParameters(args.yaml_file)
    cosmo_args.print()

    ulog_args = utils.UlogParameters(args.yaml_file)
    ulog_args.print()
    ...
    offset_cosmo_time = cosmo_args.get_cosmo_time(t0.hour) * 0
    cosmo_wind = utils.extract_cosmo_data(
        cosmo_args.params['file'],
        lat0,
        lon0,
        offset_cosmo_time,
    ...
    cosmo_corners = utils.cosmo_corner_wind(
        ...
        rotate=cosmo_args.params['rotate'] * np.pi / 180.0,
    ...
    w_ekfest = np.array([
        ulog_data['we_east'], ulog_data['we_north'], ulog_data['we_down']
        ])
    all_winds = [w_vanes, w_ekfest]
    wind_names = ['Raw vane estimates', 'On-board EKF estimate']
    try:
        filtered_wind = utils.read_filtered_hdf5(ulog_args.params['filtered_file'])
        w_filtered = []
        for wind_key in ['wind_e', 'wind_n', 'wind_d']:
            w_filtered.append(
                np.interp(
                    ulog_data['gp_time'], filtered_wind['time'], filtered_wind[wind_key]
                    )
                )
    ...
    vane_lims = plotting.vector_lims(w_vanes, axis=0)
    cosmo_lims = plotting.vector_lims(
        np.array([cosmo_wind['wind_x'], cosmo_wind['wind_y'], cosmo_wind['wind_z']]),
        axis=0
        )
    ...
    fh, ah = plotting.plot_wind_3d(
        ...
        h_terr,
        cosmo_wind,
        origin=plane_pos[:, 0].flat,
        Vlims=Vlims,
        plot_cosmo=True
        )
    plot_time = (ulog_data['gp_time'] - ulog_data['gp_time'][0]) * 1e-6
    f2, a2 = plotting.plot_wind_estimates(
        plot_time, all_winds, wind_names, polar=args.polar
        )
    if args.prediction:
        try:
            prediction_array = np.load(
                'data/{0}{1}.npy'.format(cosmo_args.params['_prefix'], bn)
                )
        except:
        ...
        for pred_dim in prediction_array:
            prediction_interp.append(
                RegularGridInterpolator((z_terr2, y_terr2, x_terr2), pred_dim)
                )

        # Get all the in bounds points
        inbounds = np.ones(ulog_data['x'].shape, dtype='bool')
        inbounds = np.logical_and.reduce([
            ulog_data['x'] > x_terr[0], ulog_data['x'] < x_terr[-1], inbounds
            ])
        inbounds = np.logical_and.reduce([
            ulog_data['y'] > y_terr[0], ulog_data['y'] < y_terr[-1], inbounds
            ])
        inbounds = np.logical_and.reduce([
            ulog_data['alt'] > z_terr[0], ulog_data['alt'] < z_terr[-1], inbounds
            ])

        pred_t = (ulog_data['gp_time'][inbounds] - ulog_data['gp_time'][0]) * 1e-6
        points = np.array([
            ulog_data['alt'][inbounds], ulog_data['y'][inbounds],
            ulog_data['x'][inbounds]
            ]).T
        pred_wind = [
            prediction_interp[0](points), prediction_interp[1](points),
            prediction_interp[2](points)
            ]

        if args.polar:
            pred_V, pred_dir = rec2polar(
                pred_wind[0], pred_wind[1], wind_bearing=True, deg=True
                )
    ...
    if w_filtered is not None:
        ...
        w_plot = w_vanes
    fp, ap = plotting.plot_vertical_profile(
        z_terr, cosmo_corners[:, :, cy, cx], w_plot, ulog_data['alt'], plot_time
        )
    fp.set_size_inches((5, 8))
    fv, av = plotting.plot_lateral_variation(
        w_plot, plane_pos, plot_time, min_alt=1700, max_alt=None
        )

    if args.save_figs:
        print("Saving figures.")
        fh.savefig(
            'fig/{0}{1}_wind3d.png'.format(cosmo_args.params['prediction_prefix'], bn),
            bbox_inches='tight'
            )
        f2.savefig(
            'fig/{0}{1}_wind.png'.format(cosmo_args.params['prediction_prefix'], bn),
            bbox_inches='tight'
            )
        fp.savefig(
            'fig/{0}{1}_windProfile.png'.format(
                cosmo_args.params['prediction_prefix'], bn
                ),
            bbox_inches='tight'
            )
        fv.savefig(
            'fig/{0}{1}_windLateral.png'.format(
                cosmo_args.params['prediction_prefix'], bn
                ),
            bbox_inches='tight'
            )
